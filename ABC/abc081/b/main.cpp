...
int main() {
    int N, A[200]; // 配列サイズ200で初期化（問題の制約に合わせて調整）
    cin >> N;      // 配列のサイズ N を入力

    // 配列の各要素を入力
    for (int i: range_half_open(0, N)) {
        cin >> A[i];
    }

    // 配列の各要素が 2 で割れる最大回数を記録する変数
    int min_shifts = INT_MAX; // 最小シフト回数を求めるため、初期値を最大値に設定

    // 各要素に対して 2 で割れる回数を計算
    for (int i: range_half_open(0, N)) {
        // この要素に対するシフト回数を記録
        int shifts = 0; 
        // 偶数である限りループ（ビット演算でチェック）
        while ((A[i] & 1) == 0) {
            // A[i] /= 2 と同じ意味
            A[i] >>= 1;
            shifts++;
        ...
        // 配列中に奇数があれば、それ以上の計算は不要
        if (min_shifts == 0) break;
    }
...
public:
    ...
    iterator begin() const {
        return iterator(start, stop, is_closed);
    }

    iterator end() const {
        return iterator(is_closed ? stop + 1 : stop, stop, is_closed);
    }
};
