...
public:
    Range(T start, T end, bool is_closed = false)
        : start(start), end(end), is_closed(is_closed) {}
    ...
    public:
        iterator(T current, T end, bool is_closed)
            : current(current), end(end), is_closed(is_closed) {}
        ...
        bool operator!=(const iterator& other) const {
            return is_closed ? current <= other.current : current < other.current;
        }
    };

    iterator begin() const {
        return iterator(start, end, is_closed);
    }
    iterator end_iter() const {
        return iterator(end + is_closed, end, is_closed);
    }
};

template <typename T>
inline Range<T> range_half_open(T start, T end) {
    return Range<T>(start, end, false);
}

template <typename T>
inline Range<T> range_closed(T start, T end) {
